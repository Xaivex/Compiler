...
class FnBodyNode extends ASTnode {
    ...
    public void typeCheck(Type fnRetType) {
        myStmtList.typeCheck(fnRetType);
    }
...
class IfElseStmtNode extends StmtNode {
    ...
    public void typeCheck(Type ret) {
        ...
        myThenStmtList.typeCheck(ret);
        myElseStmtList.typeCheck(ret);
        return;
    }
...
class AssignNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        if(rightType.isErrorType() || (leftType.isErrorType()))
            return new ErrorType();

        if(!(rightType.isErrorType()) && !(leftType.equals(rightType))) {
...
class CallExpNode extends ExpNode {
    ...
    public Type typeCheck() {
        ...
        if (myExpList.theExps().size() != expected) {
            ...
            List<Type> formTypes = ((FnSym)myId.sym()).getParamTypes();
            List<ExpNode> actTypes = myExpList.theExps();
            for (int i = 0; i < formTypes.size(); i++) {
                ExpNode tExp = actTypes.get(i);
                Type tType = tExp.typeCheck();
                Type fType = formTypes.get(i);
                if (!fType.equals(tType)) {
                    if (tType.isErrorType()) {
                        continue;
                    }
                    ErrMsg.fatal(tExp.lineNum(), tExp.charNum(), "Type of actual does not match type of formal");
                }
            }
        }
